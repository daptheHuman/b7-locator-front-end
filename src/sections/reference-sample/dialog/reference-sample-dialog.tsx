...
import { createReferencedSample } from '../api/referenced-samples';
...
const RetainedSampleDialog = ({
  ...
  const [values, setValues] = React.useState<CreateReferencedSample>({
    product_code: '',
    batch_number: '',
    rack_id: '',
  ...
  const createSampleHandler = () => {
    createReferencedSample(values).then(() => {
      handleOpenClose();
      fetch();
    });
  };
